...
const FormInput: React.FC<Props> = forwardRef((props, ref) => {
  ...
  const [fieldInfoCreateModalVisible, setFieldInfoCreateModalVisible] = useState(false)
  const [tableInfoCreateModalVisible, setTableInfoCreateModalVisible] = useState(false)
  const [createFieldInfo, setCreateFieldInfo] = useState<FieldInfoType.FieldInfo>()
  const [createTableInfo, setCreateTableInfo] = useState<TableInfoType.TableInfo>()
  const [importFieldDrawerVisible, setImportFieldDrawerVisible] = useState(false)
  // 导入字段的位置
  ...
  return (
    <>
      <Form<TableSchema> className='font-input' form={form} scrollToFirstError
        onFinish={onFinish} onReset={() => { form.resetFields(['fieldList']) }}
      >
        <Form.Item name='dbName' label='库名'>
          <Input placeholder='多个单词间建议用下划线分割'></Input>
        </Form.Item>
        <Form.Item
          name="tableName"
          label="表名"
                                ...
                                <Select style={{ width: 150 }} showSearch
                                  dropdownRender={menu => (
                                    <>
                                      ...
                                      <Space align='center' size={24} style={{ marginLeft: 8 }}>
                                        <Button size='small'
                                          onClick={() => {
              ...
              <Form.Item>
                ...
                >
                  ...
                  <Button type='dashed' block
                    icon={<PlusOutlined />}
                    onClick={() => {
                      setImportIndex(form.getFieldsValue().fieldList?.length ?? 0)
